...
class AirlinesLoader
  ...
  def import_file(file_path)
    ...
    @logger.info("Importing airlines from #{file_path}")
    ...
    File.open(file_path, 'rb') do |file|
      ...
      begin
        ...
        csv.each do |row|
          ...
          iata_code = presence(row[3])
          airline_name = presence(row[1])

          # Skip rows without IATA code or airline name
          next if iata_code.nil? || iata_code.empty? || iata_code == '\\N'
          next if airline_name.nil? || airline_name.empty?

          doc = {
            'Reporting_Airline' => iata_code,
...
def parse_options(argv)
  options = {
    ...
    airlines_file: 'data/airlines.csv.gz',
    index: 'airlines',
    batch_size: AirlinesLoader::BATCH_SIZE,
    refresh: false,
    delete_index: false
  }

  parser = OptionParser.new do |opts|
    opts.banner = 'Usage: import_airlines.rb [options]'

    opts.on('-c', '--config PATH', 'Path to Elasticsearch config YAML (default: config/elasticsearch.yml)') do |path|
      options[:config] = path
    end

    opts.on('-f', '--file PATH', 'Path to airlines CSV file (default: data/airlines.csv.gz)') do |path|
      options[:airlines_file] = path
    end

    opts.on('--index NAME', "Override index name (default: airlines)") do |name|
      options[:index] = name
    end
...
def main(argv)
  ...
  airlines_file = resolve_file_path(options[:airlines_file])

  loader = AirlinesLoader.new(
    client: client,
    index: options[:index],
    logger: logger,
  ...
  loader.ensure_index
  loader.import_file(airlines_file)
end
